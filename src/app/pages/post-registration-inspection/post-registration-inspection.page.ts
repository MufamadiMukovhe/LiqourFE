...
import { ComplianceInspection } from 'src/app/model/model';
...
@Component({
  selector: 'app-post-registration-inspection',
  templateUrl: './post-registration-inspection.page.html',
  styleUrls: ['./post-registration-inspection.page.scss'],
})
export class PostRegistrationInspectionPage implements OnInit {
  @Input() showFailedCompliance: boolean = false;
  currentForm: string = 'landing';
  @Input() h1Title: string = 'Complete Compliance Inspection';
  @Input() action: any = '1';
  ...
  constructor(
    ...
    private service2: GeneralService,
...
caseNo: any;
  ...
  ngOnInit() {
    ...
    this.getOutletInfo(this.caseId); 
...
isFormValid():boolean{
  ...
  compliance = new ComplianceInspection()
  submitComplianceInspection(): void {
    this.healthCheckForm.markAllAsTouched();
    ...
    const formData = {
      ...
      healthCheckData: this.healthCheckForm.value.questions.map((question: { answer: any; }, index: number) => ({
        question: this.questions[index].question,
        answer: question.answer || "" // User's answer
      })),
      ...this.recommendationForm.value,
      typeOfComplianceInspection:this.action
    };

    this.service.complianceInspection(this.caseId, formData).subscribe({
      next: (res: any) => {
        this.spinner.hide();
  ...
  private getOutletInfo(outletId: any) {
    this.spinner.show()
    this.service2.getOutletInfoPostReg(outletId).subscribe({
      next: (res: any) => {
        this.spinner.hide()
